...
if (isset($_POST['update_status'])) {
    ...
    if ($report_id > 0) {
        $update_stmt = $conn->prepare("UPDATE organization_reports SET status = ? WHERE id = ?");
        if ($update_stmt) {
            $update_stmt->bind_param("si", $new_status, $report_id);
            $update_stmt->execute();
            $update_stmt->close();
            
            header("Location: admin-organizations.php?tab=" . urlencode($role_tab) . "&page=" . $current_page . "&limit=" . $current_limit . "&success=" . urlencode("Organization report status updated successfully!"));
            exit();
        }
    }
    
    header("Location: admin-organizations.php?tab=" . urlencode($role_tab) . "&error=" . urlencode("Failed to update report status."));
    exit();
}

// 2. Handle File Upload & Organization Report Creation
if (isset($_POST['upload_org_report'])) {
    ...
    if (!empty($org_name) && !empty($academic_yr) && !empty($semester) && !empty($report_title) && isset($_FILES['report_file']) && $_FILES['report_file']['error'] === UPLOAD_ERR_OK) {
        $file_name  = $_FILES['report_file']['name'];
        $file_tmp   = $_FILES['report_file']['tmp_name'];
        $upload_dir = '../uploads/organization_reports/';
        ...
        if (move_uploaded_file($file_tmp, $target_file)) {
            $insert_stmt = $conn->prepare("INSERT INTO organization_reports (org_name, academic_year, semester, report_title, total_members, file_path, status, created_at) VALUES (?, ?, ?, ?, ?, ?, 'Pending', NOW())");
            if ($insert_stmt) {
                $insert_stmt->bind_param("ssssis", $org_name, $academic_yr, $semester, $report_title, $total_members, $new_filename);
                $insert_stmt->execute();
                $insert_stmt->close();
                
                header("Location: admin-organizations.php?tab=" . urlencode($role_tab) . "&success=" . urlencode("Organization accomplishment report uploaded successfully!"));
                exit();
            }
        }
        
        header("Location: admin-organizations.php?tab=" . urlencode($role_tab) . "&error=" . urlencode("Failed to move uploaded file to destination server."));
        exit();
    } else {
        header("Location: admin-organizations.php?tab=" . urlencode($role_tab) . "&error=" . urlencode("Please complete all required fields and upload a valid report file."));
        exit();
    }
}
...
$countQuery = "SELECT COUNT(*) as total FROM organization_reports {$whereClause}";
...
$dataQuery = "SELECT * FROM organization_reports {$whereClause} ORDER BY created_at DESC LIMIT ? OFFSET ?";
...
$stats = $stat_result ? $stat_result->fetch_assoc() : ['total' => 0, 'total_members' => 0, 'pending' => 0, 'approved' => 0];
?>
...
<head>
    ...
    <meta name="description" content="Organizations Management - College of Criminal Justice">
...
<body>
        ...
        <main class="dashboard-container" id="mainContent" role="main">
            ...
            <section class="card welcome-card" aria-label="Welcome Section">
                <div class="welcome-content">
                    ...
                    <p>Upload and review student organization accomplishment reports, rosters, and event documentations.</p>
                </div>
            </section>
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <p>TOTAL MEMBERS / PARTICIPANTS</p>
                    <h3><?= intval($stats['total_members'] ?? 0); ?></h3>
                </div>
                <div class="stat-card">
                    <p>PENDING REVIEWS</p>
            ...
            <div class="concerns-tabs-container" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
                <div class="tabs-group">
                    <a href="admin-organizations.php?tab=All&limit=<?= $limit ?>" class="concern-tab <?= $active_tab === 'All' ? 'active' : ''; ?>">All</a>
                    <a href="admin-organizations.php?tab=1st+Semester&limit=<?= $limit ?>" class="concern-tab <?= $active_tab === '1st Semester' ? 'active' : ''; ?>">1st Semester</a>
                    <a href="admin-organizations.php?tab=2nd+Semester&limit=<?= $limit ?>" class="concern-tab <?= $active_tab === '2nd Semester' ? 'active' : ''; ?>">2nd Semester</a>
                    <a href="admin-organizations.php?tab=Summer&limit=<?= $limit ?>" class="concern-tab <?= $active_tab === 'Summer' ? 'active' : ''; ?>">Summer</a>
                </div>
                <button class="modal-btn-save" onclick="openUploadModal()" style="display: inline-flex; align-items: center; gap: 8px; padding: 10px 18px; text-decoration: none; cursor: pointer;">
                    <i class="fa-solid fa-cloud-arrow-up"></i> Upload Org Report
                </button>
            </div>

            <section class="card table-container" aria-label="Organizations Reports List" style="padding: 0; overflow: hidden;">
                <div class="table-wrapper">
                        ...
                        <table>
                            ...
                            <tbody>
                                <?php foreach ($submissions as $row): ?>
                                    <tr>
                                        <td data-label="Organization Name"><strong><?= htmlspecialchars($row['org_name'] ?? 'N/A'); ?></strong></td>
                                        ...
                                        <td data-label="Members / Attendees"><?= htmlspecialchars($row['total_members'] ?? 0); ?> Students</td>
                                        <td data-label="Date Uploaded"><?= !empty($row['created_at']) ? date("M d, Y h:i A", strtotime($row['created_at'])) : 'N/A'; ?></td>
                                        <td data-label="Attachment">
                                            <?php if (!empty($row['file_path'])): ?>
                                                <a href="../uploads/organization_reports/<?= htmlspecialchars($row['file_path']); ?>" target="_blank" class="document-link" title="View Document">
                    ...
                    <?php else: ?>
                        <div class="no-results" style="padding: 40px; text-align: center;">
                            ...
                            <p>No organization reports found under <?= htmlspecialchars($active_tab); ?>.</p>
                        </div>
                    <?php endif; ?>
                </div>
            </section>
    ...
    <!-- Upload Organization Accomplishment Report Modal -->
    <div id="uploadModal" class="modal-overlay">
        <div class="modal-content-box" style="max-width: 550px;">
            <h3 class="modal-title"><i class="fa-solid fa-cloud-arrow-up"></i> Upload Organization Report</h3>
            <form action="admin-organizations.php" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="upload_org_report" value="1">
                <input type="hidden" name="active_tab" value="<?= htmlspecialchars($active_tab); ?>">

                <div class="modal-form-group">
                    <label class="modal-label">Organization Name <span style="color:red;">*</span></label>
                    <input type="text" name="org_name" class="modal-select" placeholder="e.g. Criminology Student Council" required style="width: 100%; box-sizing: border-box;">
                </div>
                ...
                <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 12px;">
                    <div class="modal-form-group">
                        ...
                        <input type="text" name="report_title" class="modal-select" placeholder="e.g. Annual Accomplishment Report" required style="width: 100%; box-sizing: border-box;">
                    </div>

                    <div class="modal-form-group">
                        <label class="modal-label">Total Members <span style="color:red;">*</span></label>
                        <input type="number" name="total_members" class="modal-select" placeholder="e.g. 120" required style="width: 100%; box-sizing: border-box;">
                    </div>
                </div>
    ...
    <div id="reviewModal" class="modal-overlay">
        <div class="modal-content-box">
            ...
            <form action="admin-organizations.php" method="POST">
