...
$concerns = [];
if (!empty($alumni_no)) {
    $stmt = $conn->prepare("SELECT * FROM alumni_concerns WHERE alumni_no = ? ORDER BY id DESC");
    if ($stmt) {
        $stmt->bind_param("s", $alumni_no);
        $stmt->execute();
        $result = $stmt->get_result();
                                    ...
                                    <tr>
                                        <td>
                                            <?php if (!empty($row['file_path'])): ?>
                                                <a href="../uploads/alumni_concerns/<?= htmlspecialchars($row['file_path']); ?>" target="_blank" style="color: #2563eb; text-decoration: underline;">View File</a>
                                            <?php else: ?>
                                                N/A
                                            <?php endif; ?>
                                        </td>
